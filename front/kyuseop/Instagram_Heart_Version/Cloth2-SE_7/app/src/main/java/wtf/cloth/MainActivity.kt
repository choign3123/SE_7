...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //데이터 클래스 사용?
        val clthInfo1 = ClothInformation(1,false,"상의","봄")
        //이거 쓰지 말까.
        var clthIdx: Int = 1
        ...
        if (intent.hasExtra("ModifiedClothInfo")) {
            ...
            clthInfo1.bookmark = intent.getBooleanExtra("mdfbookmark",false)
            clthInfo1.category = intent.getStringExtra("mctgr")
            clthInfo1.season = intent.getStringExtra("mss")
        }
        if (clthInfo1.bookmark==true) {binding.favorite.text = "즐겨찾기 등록됨."};
        else if(clthInfo1.bookmark==false){binding.favorite.text = "즐겨찾기 해제됨."}
